...
#create a dataset
makeDat <- function(n, p, ncomp, variances){
...
# call to use an existing data set to test the performance
test_data <- function(name){
  if(name == 'iris'){
    
    data('iris')
    X <- drop_na(iris[1:4])
    X <- matrix(unlist(X), nrow = dim(X)[1], ncol = dim(X)[2])
    return(X)
  }
  ...
  if(name == 'pitprops'){
    ...
    data('pitprops')
    X <- matrix(pitprops, nrow = dim(pitprops)[1], ncol = dim(pitprops)[2])
    print('WARNING: dat is correlation matrix')
    return(X)
  }
}
