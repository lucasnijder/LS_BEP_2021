# set working directory to folder with source files
setwd("~/datascience4/BEP/R")
...
file_list <- c('lucas_make_data.R', 'lucas_run_pca.R', 'lucas_run_spca.R', 'lucas_pc_select.R')
...
run_simulation <- function(sim_param_combs, variance_of_comps, nrFolds){
  ...
  return(c(num_mc = num_mc, p = p, error = e, n = n, ncomp = c, pca_ncomp = res_comp_pca, spca_ncomp = res_comp_spca))
}

tic('run_simulation')
sim_param_combs <- expand.grid(seq(1, MC_reps), num_var_seq, num_comp_seq, error_seq, n_seq)
results <- apply(X = sim_param_combs, MARGIN = 1, FUN = run_simulation, variance_of_comps=variance_of_comps, nrFolds=nrFolds)
toc()

stopCluster(cl)
print('done')

data.frame(MC=results[1,],p=results[2,],error=results[3,],n=results[4,],ncomp=results[5,],pca_ncomp=results[6,],spca_ncomp=results[7,],pca_corect=results[8,],spca_correct=results[9,])
